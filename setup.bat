...
if exist "%SCRIPT_DIR%package.json" (
    ...
    echo.
    echo 次のいずれかの場所に setup.bat を置いてください。
...
for /l %%i in (1,1,60) do (
    powershell -NoProfile -Command "try { $c = New-Object System.Net.Sockets.TcpClient; $c.Connect('127.0.0.1', %DEV_PORT%); $c.Close(); exit 0 } catch { exit 1 }" >nul 2>&1
    if not errorlevel 1 (
        set "SERVER_READY=1"
        goto :server_ready
    )
